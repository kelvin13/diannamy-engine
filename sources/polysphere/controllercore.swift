...
struct GlobeView<Model> 
{
    private 
    enum Action:Equatable
    {
        case none
        case selectNode((Int, Int)) 
        case moveNode((Int, Int), Vector3<Float>) 
        case addNode((Int, Int), Vector3<Float>)
    }
    
    @State.Binding 
    var model:Model
    var key:WritableKeyPath<Model, [[Vector3<Float>]]>
    {
        didSet 
        {
            self.sequence = nil 
    ...
    private 
    var traces:[[Vector2<Float>]]
    
    private 
    var action:Action 
    
    init(model:State.Binding<Model>, key:WritableKeyPath<Model, [[Vector3<Float>]]>) 
    {
        self.$model     = model 
        self.key        = key
        self.sequence   = nil 
...
{
    var display:(viewport:Vector2<Float>, frame:Rectangle<Float>, plane:ControlPlane) 
    ...
    init() 
    {
        let rig:Camera.Rig =  .init(center: .zero,
                               orientation: .identity,
                                  distance: 4,
                               focalLength: 32)
        
        self.display = (.zero, .zero, .init(rig))
        
        let runs:[UI.Text.Run] = 
        [
            .init("hello world ",           selector: .paragraph | .strong), 
            .init("to all who inhabit it",  selector: .paragraph), 
        ]
        let text:UI.Text = .init(runs, selector: .paragraph)
